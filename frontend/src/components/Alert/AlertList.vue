<template>
  <div class="alert-container" v-show="alerts.length">
    <div class="alert-container-inner">
      <transition-group enter-active-class="animated slideUp" leave-active-class="animated fadeOut" :duration="350" mode="out-in">
        <x-alert-item :alert="alert" v-for="(alert, idx) in alerts" :key="alert.id" @close="onClose(idx)"></x-alert-item>
      </transition-group>
    </div>
  </div>
</template>
